...
export class Tab3Page implements OnInit, AfterContentChecked {
	...
	ngOnInit() {
		...
		this.userChampionnats = this.http.get('../../assets/mocks/userChamp.json')
			.pipe(
				tap(r => console.log(r))
			);
		this.championnats = this.http.get('../../assets/mocks/championnats.json')
			.pipe(
				map(r => _.filter(r, ['type', 'Friends&Familly'])),
				tap(r => console.log(r))
			);
		this.championnatsNetwork = this.http.get('../../assets/mocks/championnats.json')
			.pipe(
